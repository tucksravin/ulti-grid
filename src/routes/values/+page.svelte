<script lang="ts">
  ...
  const handleSwipe = (
    ...
    if (direction == "right" && activeValue > 1) activeValue--;
    if (direction == "left" && activeValue < 3) activeValue++;
  };
</script>
...
<ContentWidth>
  ...
  <div
    use:swipe={{ minSwipeDistance: 20, touchAction: "pan-y" }}
    onswipe={handleSwipe}
    class="w-[300vw] lg:translate-x-0 lg:w-full flex flex-row justify-around lg:justify-between flex-nowrap transition-transform overflow-hidden duration-1000"
  ...
  <div
    class="absolute h-10 flex align-middle justify-start xl:left-8 translate-x-[2px] -bottom-4 lg:hidden"
  >
    <div
      class="h-[10px] w-[10px] border-[1.5px] rounded-full transition-all duration-1000 cursor-pointer hover:opacity-60 mr-4 {activeValue ===
      1
        ? 'bg-dark border-dark'
        : 'border-light'}"
  ...
  <button
    onclick={() => activeValue--}
    class="{activeValue == 1 || innerWidth > 1024
      ? 'hidden'
      : ''} absolute left-0 h-6 w-6 rounded-full border-[#C2D1D9] border-2 p-1 flex align-middle justify-center cursor-pointer transition-all duration-500 hover:bg-[#424B5A] hover:border-[#424B5A]"
  >
    <img alt="chevron-left" src={chevronLeft} class="-translate-x-[1px]" />
  </button>
  <button
    onclick={() => activeValue++}
    class="{activeValue == 3 || innerWidth > 1024
      ? 'hidden'
      : ''} absolute right-0 h-6 w-6 rounded-full border-[#C2D1D9] border-2 p-1 flex align-middle cursor-pointer transition-all duration-500 justify-center hover:bg-[#424B5A] hover:border-[#424B5A]"
  >
